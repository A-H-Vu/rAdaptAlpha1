...
expName = 'rAdaptAlpha1'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Andrew\\Documents\\Work\\Projects\\rAdaptAlpha1\\rAdaptAlpha1_lastrun.py',
...
# Set up trimmed width and height
    ...
    for thisTrialsLoop in trialsLoop:
        ...
        while continueRoutine:
            ...
            if trialCursor.status == STARTED:  # only update if drawing
                ...
                trialCursor.setPos([cursorPosY, cursorPosY], log=False)
            ...
            if trialBuff.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > trialBuff.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    trialBuff.tStop = t  # not accounting for scr refresh
                    trialBuff.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(trialBuff, 'tStopRefresh')  # time at next scr refresh
                    trialBuff.setAutoDraw(False)
